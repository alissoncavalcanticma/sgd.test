...
function getitens(pag, maximo){
	...
	//url:'getitens.php',
	url:'../controller/AcessoController.class.php',
...
function paginador(cont){
	...
	if(cont<=numitens){ //Verificando se há mais de uma página
		...
		$('#listarAcessos').html('<tr><td>Apenas uma Página<td><tr>');

	}else{

		$('#listarAcessos').html('<tr class="pagination pg-blue pagination"></tr>');

		if(pagina!=1){

			$('#listarAcessos tr').append('<td><a  class="page-link" aria-label="Previous" href="#" onclick="getitens('+(pagina-1)+', '+numitens+')"> << </a></td>')
		}
		...
		if(pagina!=qtdpaginas){
			$('#listarAcessos tr').append('<td><a class="page-link" aria-label="Next" href="#" onclick="getitens('+(pagina+1)+', '+numitens+')"> >> </a></td>')
		}
		//console.log(cont);
	}
}
...
function paginadorPesquisa(cont, valores, pagina, maximo){
	...
	if(cont<=numitens){ //Verificando se há mais de uma página
		...
		if(pagina!=1){
			...
			$('#listarAcessos tr').append('<td><a  class="page-link" aria-label="Previous" href="#" onclick="getPesquisa('+(pagina-1)+', '+numitens+', '+valores+')"> << </a></td>')
		}
		...
		if(pagina!=qtdpaginas){
			$('#listarAcessos tr').append('<td><a class="page-link" aria-label="Next" href="#" onclick="getPesquisa('+(pagina+1)+', '+numitens+', '+valores+')"> >> </a></td>')
		}
		console.log(cont);
	}
}
